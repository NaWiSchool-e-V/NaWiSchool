...
const char* label[] = { " Temp.:", " Hum.:", " Time:", " Bat.:" };
const char* units[] = { "C", "%", " ", "%" };
...
void setup() {
  ...
  logfile.println("Name: AdaloggerV1, HW Version: 1.0, SW Version: 1.4, reduced Resolution");
  logfile.println("CPU time in ms, date, hh:mm:ss(UTC), temp, press, hum, lat, lng, alt, speed, course, distanceToStart, satellites");
  if (!logfile) {
#ifdef ENLOG
  ...
  Serial.println(filename);
  Serial.println("CPU time in ms, date, hh:mm:ss(UTC), temp, press, hum, lat, lng, alt, speed, course, distanceToStart, satellites");
#endif
  logfile.close();
