...
int definedDelay = 16;          //set Delay between measurements
...
const char* label[] = { " Temp.:", " Hum.:", " Time:", " S./B.:" };
const char* units[] = { "C", "%", " ", " " };
...
void setup() {
  ...
  Serial.println("\r\nAdalog logger test");
  Serial.println("BME280 Adalogger By Tim Schumann");
  Serial.println();
#endif
  pinMode(A3, INPUT);
  ...
  logfile.println("Name: AdaloggerV1, HW Version: 1.0, SW Version: 1.6, reduced Resolution by Tim Schumann");
  logfile.println("#, CPU time in ms, hh:mm:ss(UTC), date, temp, press, hum, gas, lat, lng, alt, speed, course, distanceToStart, satellites, batteryVoltage");
  if (!logfile) {
#ifdef ENLOG
  ...
  Serial.println(filename);
  Serial.println("Name: AdaloggerV1, HW Version: 1.0, SW Version: 1.4, reduced Resolution");
  Serial.println("#, CPU time in ms, hh:mm:ss(UTC), date, temp, press, hum, gas, lat, lng, alt, speed, course, distanceToStart, satellites, batteryVoltage");
#endif
  logfile.close();
...
void sdwrite() {
  ...
  if (count % 10 == 0) displayState = !displayState;
